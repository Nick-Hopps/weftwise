...
| `query-prompt.ts` | 回答用户问题 + 引用；`buildQueryUserPrompt` 加可选 history transcript。`buildQueryAgenticSystemPrompt({ mode, imageInsertEnabled })` 按真实 ToolSet 生成工具说明，并约束 current/cross read 引用与跨 Subject 只读。另含统一意图 `QueryIntentSchema/QUERY_INTENT_SYSTEM_PROMPT/buildQueryIntentUserPrompt`：一次分类 read/propose、直接 Re-enrich、选区配图与 reset phase；以及 `buildAgenticUserContent` 和独立 coverage 判定。 |
