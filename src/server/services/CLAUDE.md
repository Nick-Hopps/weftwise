...
- ⑨ 提交前若 `ctx.citedSources` 非空：一次性 `extractContent(全部 url)`（失败回落 snippet）→ 纯函数 `buildWebSourceImports`（`saveSource` 包 `saveRawSource`，单源失败跳过）→ 把网页源 `{ links, extraStagePaths }` 作 `commitPending` 第三参（`saveRawSource` 导入 source 实体；`extraStagePaths`=raw 文件+sidecar 进同一 commit；`links`→`page_sources`）。导出纯函数 `filenameFromUrl` / `buildWebSourceImports` / `deriveWebSourcePresentation`（标题取搜索结果标题、缺失回退 hostname；描述取 snippet、缺失回退正文首个非导航段落——写入 source 展示元数据后侧栏直接显示网页标题与描述）。
...
- **联网来源同构**：`extractWebCitationsFromAnswer(answer, accessed)`（同文件，纯函数）扫描答案里出现的所有 http(s) URL，与 `accessed.webResults` 求交——模型凭空写的 URL 一律不进来源。**不解析 markdown 语法**：`[t](url)`、裸 URL、`<url>`、括号包裹都被同一次 URL 扫描命中，收尾括号与句末标点交给 `normalizeCitationUrl` 的括号平衡规则剥离，覆盖面比按 markdown 结构解析更广。精确匹配之外有一层 **`origin+pathname` 回退**：实测 Tavily 会返回带跟踪参数的 URL（`…/a/20260720A09YMY00?uid[0]=…`）而模型按纪律抄的是干净 URL，只做精确匹配会把**真实来源当幻觉丢掉**；回退仅在「答案侧无 query」且「该路径只有唯一候选」时生效，多义一律不猜（否则 `?id=1` 与 `?id=2` 会被错认成同一篇）。标题取 `webResults` 里的服务端记录，**不取模型写的锚文本**（锚文本可与目标页无关）；刻意不给 web 条目做 excerpt——搜索引擎摘要不具备 wiki excerpt「恒为页面原文字面子串」的保证，混在一起会拉平两者的可信度档位；
...
- 任务 `save-to-wiki`：同时支持 `params.subjectId`（来自 body）与 `job.subjectId`（来自 enqueue）；`saveQueryAsPage` 经纯函数 `buildReferencesSection(citations, webCitations, subjectSlug)` 组装 answer + `## References` 正文（wiki 引用写 wikilink、网页来源写 `- [标题](url)`，wiki 在前；两侧皆空则整节不出现）和 `query-answer` tag，再调用 `page-write::createPageInSubject(..., { jobId })`，与 `wiki.create` 共用唯一 slug、create plan/apply、Saga 和 embedding 回填。页面 citation 只保留为正文 wikilink，frontmatter `sources` 继续专用于 raw source ID（本路径为空）。若 worker 在 commit 后、job complete 前重试，则按该 job 的 applied operation 恢复唯一 canonical create slug、核对页面仍存在并补 enqueue embedding，不创建后缀重复页。
...
- `AccessedPages.webResults` — 本轮 `web.search` **真实返回过**的网页（key 为 `normalizeCitationUrl` 规范化 URL，首次写入优先）。`webSearch` 包装只留痕、不改动交给模型的返回值，职责层级与 `onSourceAccess` 相同；未配置联网检索时该 Map 恒空，全链路行为与改动前一致。
...
手动 Health check 使用 `discovery`，执行确定性扫描与开放式语义发现。Fix/Curate 完成后不再创建 lint job：任务内 `verifyJobPostconditions` 已产出 `perFindingOutcomes`，`remediation-status.ts` 在读取原 lint 快照时把 baseline 之后已完成验证且**确实修好**（fixed）的关联 finding 移除并重算 severity，真实结果保留在近期摘要；**未解决的 skipped / failed 留在列表并标注**（见下方状态恢复段）。Research 等 provenance run 到达验证终态后执行同样投影。若用户之后手动 discovery 又发现同一 finding，较新的 lint 结果优先并重新展示。`/api/lint` 的显式 `verification` 模式与 `lint-verification.ts` 暂留作旧客户端兼容，不再由 Health UI 调用。
...
**状态恢复（`remediation-status.ts`）**：`buildHealthSnapshot()` 将当前 lint、router plan、近期 jobs 与 route 批量注入的 `ResearchRunView[]` 合成为 `HealthSnapshot`，自身保持纯函数。`MAX_REMEDIATION_JOBS = 200`；API 先有界取 jobs，再按 subject 批量读取关联 run，避免逐 finding 查询。无 job 使用 router 初始状态；pending/running 为 `queued`。Research run 映射为：`awaiting-approval` 保持待批，`importing/verifying` 为 queued，`dismissed/empty` 为 skipped，`completed/partial/failed` 对 finding run 读取已物化的逐 finding `fixed/residual/unverifiable`，topic run 按整体终态映射；没有持久化 run 的历史 Research job 继续从旧 `resultJson.candidates` 保守回退。Fix / Curate 优先读取 `perFindingOutcomes`。关联处置在当前 lint 快照之后完成自身验证后，**只有 `fixed` 才从当前 findings 移除**并把真实结果写入 `recentOutcomes`；**Fix/Curate 的 `skipped`（未触达）与 `failed`（处置没成功）语义都是「问题还在原地」，finding 保留在列表并由 `applyCurrentJob` 标注真实状态，不进 `recentOutcomes`**（`isUnresolvedOutcome`，2026-07-30 从只放过 `skipped` 的 `isUntouchedSkip` 扩展而来）——隐藏它们等于谎报已处理，用户点完 Tidy 看到问题消失、下次 discovery 又原样发现同一稳定 ID。Research 的 `skipped` 来自 dismissed/empty run（用户显式忽略或确无候选），属于已处理，仍随 provenance run 到达验证终态后一并移除。更新的手动 lint 可重新发现并展示同一 finding，损坏或缺少验证结果的 completed job 不得误隐藏。
...
**Fix scope**：`remediationContext` 存在时只从指定、同 subject、completed lint job 解析所选 ID；确定性检查仍新鲜重扫，但只消费 ID 命中的 `missing-frontmatter/broken-link`，语义侧只消费 ID 命中的 `missing-crossref/contradiction`。工具 read/search/inspect/source evidence 保持 subject-wide，`updatePage/patchPage/linkEnsure(source)` 写侧收窄到 `fix-deterministic::fixWritableSlugs(worklist)`：所选 findings 的 `pageSlug`，**contradiction 额外并入 `evidence[].pageSlug`** —— 矛盾天生跨两页，而错的那页往往是对侧页（`pageSlug` 只是 lint 挂载点），只按 pageSlug 开白名单会让模型判对了也写不进去（2026-07-30 修复）。`missing-crossref` 虽也带 evidence 但不扩：它的写入落点就是源页。白名单同时经 `buildFixAgenticUserPrompt` 的 **Writable pages** 一节事前告知模型，`scopeFixWrites` 的拒绝消息也带上清单；被拒的 slug 记进闭包并 emit `fix:scope-blocked`（`fix:tool` 走 AI SDK `onStepFinish`，只报发起了哪些调用、拿不到结果，被拒写入在日志里与成功写入长得一样）。无 context 的旧 `/api/fix` 继续保持全量 Fix 行为，不收窄写侧、不输出该节。
...
1. 解析 scope + seedSet：`scope:'pages'`（auto）→ seed = params.slugs，再用 `expandScopeWithNeighbors` 扩展本-subject 邻居；`scope:'subject'`（manual）→ 全 subject 非 meta 页，seedSet=null（无 seed 过滤）。**带 orphan worklist 时再经 `expandScopeWithOrphanSourceCandidates` 补候选源页**：对每个孤页用「title + summary」调 `hybridRankSlugs` 取前 `ORPHAN_SOURCE_CANDIDATES=5`（过滤 meta 与孤页自身）并入 scope，emit `curate:orphan-candidates`。理由：孤页定义上没有入链，一跳图邻居只剩它自己链出去的页，与「谁该链到它」几乎无关；「哪一页最该提到它」本质是语义相似度问题。无 worklist 时**一次检索都不发**，allowedSet 逐元素不变。
...
4. 调 `generateTextWithTools('curate', { system: CURATE_AGENTIC_SYSTEM_PROMPT, messages, tools, maxSteps: 40 })` 驱动工具循环；模型自驱读页，在已有唯一自然锚点上补/调一个链接或调整 metadata，也可执行既有结构操作；所有写临界区 job-local 串行，guard 鉴权后调 page-ops 内核 + emit 事件。**带 remediation context 时，第 0 步解析出的 orphan worklist 会经 `buildCurateAgenticUserPrompt` 的 `orphans` 参注入 user prompt**（页面 + description + suggestedFix，**不含 finding ID**——归因只靠服务端 `touchedSlugs`），并显式给出「邻域内找不到自然锚点就不要写、只报告为什么找不到」的出路；无 worklist 时 prompt 逐字节不变。此前 worklist 只用于结果归因、从不进 prompt，模型在信息真空里按 `When in doubt, do NOTHING` 稳定零写入。
5. guard.totals() 非零则 enqueue embed-index。
6. 按当前 `jobId + subjectId` 的 applied operations 执行共享确定性后置校验；无 operation 返回 clean，残留或校验异常返回 `completed + residual`，不回滚或重放写入。带 remediation context 时，从其精确 completed lint 快照按稳定 ID 恢复 orphan worklist，并生成逐 finding `perFindingOutcomes`：校验异常或无法归因的 residual 全 failed；其余 residual 按 `pageSlug / relatedSlugs` 只标记相关 orphan（同页多条保守同结果）；**其余按「该孤页现在是否真有非 meta 入链」判定**（`lint-deterministic::pageHasInboundLinks`，与 `checkOrphanPages` 共用 `collectInboundSlugs` 口径）——有则 fixed，无则 skipped。**不能按 `touchedSlugs` 判**：孤页的修复天然写在**源页**上，孤页自己永远不在 touchedSlugs 里，按它判会让补链成功也被记成 skipped（2026-07-29 修复）。这个判据也更强：它验的是问题本身没了。旧/manual Curate 无 worklist，状态层继续使用 job-level 兼容判定。

**护栏（`createCurateGuard`，`wiki/curate-plan.ts`）**：
- caps 计数器：merge≤5 / split≤5 / delete≤5 / create≤5 / update≤5；metadata patch、link ensure 与 **`wiki.patch`** 成功后均记一次 update，失败不计数。
- **`wiki.patch` 的忠实度护栏**：auto/manual 都有该工具（补孤页入链时源页可能压根没提过目标概念，`link.ensure` 要求的「已存在唯一锚点」不存在）。`curate-tools.ts::patchPage` 先 `readPageInSubject` → `applyPatchEdits` 在内存算出候选正文 → `checkRewriteFidelity(…, FIDELITY_PROFILES.fix)`，**不过则一次盘都不写、不计数、emit `curate:skip`**。注意 `wiki.patch` 在 fix/query 两条既有路径上都**没有**这层护栏（精确唯一替换风险面小），Curate 无人复核才补上。该护栏只拦「正文缩水 / 丢失原有链接 / frontmatter 变动」，**拦不住**「顺手改写别处、一次插多段」——那部分只有 prompt 纪律，取舍见 `docs/specs/2026-07-29-curate-orphan-autofix.md` 的 C1。
...
3. **写后定向校验与逐 finding 归因**：`operation-scope-collector` 从 applied operations 提取实际变更范围；`postcondition-verifier` 检查 broken/dangling links、新 orphan 与悬空 page_sources；原 `contradiction/missing-crossref` 在确有写入时经 `fix-semantic-postcondition` 复用 lint 路由单次无工具复检。任何不确定性保守 residual，不触发 worker retry。`fix-service` 再以实际 worklist + postcondition + `blockedWrite` 生成 `perFindingOutcomes`：校验异常时全 failed；同 `type + pageSlug` 残留（同页同类多条也保守全失败）只标记相关 finding；语义校验失败只污染语义 finding；**「触达」按 `fixWritableSlugs([finding])` 与权威 `scope.touchedSlugs`（旧报告回退 created/updated/deleted 并集）求交判定，不按 `finding.pageSlug` 单页** —— contradiction 的修复常常只写对侧页，按 pageSlug 判会让修好了也记成 skipped（与 2026-07-29 orphan 的 `touchedSlugs` 误判同族，2026-07-30 修复）；未触达任一可写页的 finding 里，**有写入被写白名单挡下（`blockedWrite`）的 LLM 类 finding 记 failed，其余记 skipped** —— 「被门控挡住」和「模型主动留着」都表现为零写入，只有前者是缺陷，把它记成 skipped 会在 UI 上显示成「无需更改」。归因精度如实说明：被拒 slug 按定义不在任何 finding 的可写集里，无法反查为哪条而拒，**逐条处置精确、批量处置偏保守**。实际触达且无对应 residual 才是 fixed，禁止用批次总 writes 推断未触达项已修复。
...
- **清理孤儿**：对比 live page slugs，删除孤儿向量（如 page 被删）；同时调 `maturityRepo.pruneOrphans` 清理 `page_maturity` 孤儿行（删页/merge 不维护该表，残留会虚高维护 dueCount 并让 sweep 给已删页入队必失败的 re-enrich）。maturity 清理在 embedding 配置守卫**之前**执行，未配置 embedding 也生效。
...
├── query-tools.ts       # current/cross Subject 只读 ToolContext（wiki/source evidence + 可选 web.search，搜索结果留痕进 webResults）+ 复合身份 AccessedPages
...
├── fix-deterministic.ts # 🆕 纯函数：fixMissingFrontmatter / buildFixWorklist / fixWritableSlugs（写白名单：contradiction 含 evidence 对侧页）/ buildSubjectReportLines / createFixGuard（写 cap + 保护页；忠实度已收编到 wiki/rewrite-fidelity.ts）
...
| 日期 | 变更 |
|------|------|
| 2026-08-03 | Ask AI 回答的来源里可以出现**联网网页**了（此前 `web.search` 结果三处断层、结构上进不了引用链路：`AccessedPages` 不留痕、`extractCitationsFromAnswer` 只认 `[[slug]]`、prompt 还明令禁止给 web 结果任何引用格式）：`query-tools` 的 `webSearch` 包装把本轮结果记进新的 `webResults`；`citation-extract` 新增 `extractWebCitationsFromAnswer` 做「答案 URL ∩ 本轮搜索结果」的确定性求交（零额外 LLM，与 wiki 引用完全同构）；`query-prompt` 的 Web search 一节改为要求 `[title](url)` 内联标注且 URL 必须逐字原样；`saveQueryAsPage` 抽出 `buildReferencesSection` 让保存页同时含 `[[slug]]` 与 markdown 链接。**真实验收发现并修掉一处误杀**：Tavily 返回的 URL 带跟踪参数（`?uid[0]=…`）、模型抄的是干净版本，纯精确匹配把真实来源判成幻觉丢弃 —— 补 `origin+pathname` 回退（仅答案侧无 query 且路径唯一候选时生效）。真实验收：world-history 问 2026-07 萨卡拉发掘 → 2 次 `web_search` → `citations` 事件 2 条网页来源、UI「来源 2」渲染 Globe+标题+域名并新标签页打开、刷新恢复正常；`saveQueryAsPage` 真实落页（vault commit `6ee2e2d`，验收后已删除 `4454edd`）References 两类行齐备且 frontmatter `sources` 保持 `[]`。spec/plan 见 `docs/{specs,plans}/2026-08-03-ask-ai-web-sources-and-tooltip.md` |
| 2026-07-31 | **Ask AI 的 propose 路径此前一次都没成功过**：`pending-action-payload.ts` 新增 `PreviewChangeToolInputSchema`（`operation` 枚举 + 全字段可选的 strict 扁平 payload），作 `wiki.preview_change` 的模型可见 schema；判别联合 `PreviewChangeInputSchema` 只留作校验（`normalizePreviewInput` 仍是唯一校验点，合法性未放宽）。原因：判别联合的 provider JSON Schema 根节点只有 `anyOf`，OpenAI 兼容接口在注册工具阶段就拒掉整个请求，凡工具集含它的请求必错。2026-07-17 那条「配图 mode 不携带 `wiki.preview_change`，避免 provider 因其顶层 union schema 拒绝工具注册」只绕开了配图一条路、没修根因，且回归断言也只覆盖 image-insert mode（该 mode 恰好过滤掉这个工具）——现补 propose mode 的根节点断言与字段集防漂移断言。用户看到的「首问必错、重试就好」是假象：重试那句被判成 `direct-reenrich`，走 `/api/query` 不跑工具循环的确定性短路。真实验收：按生产路径编译 propose ToolSet 打真实 DeepSeek，union 变体复现逐字相同的 `Invalid schema for function 'wiki_preview_change'`、当前 schema 返回 `OK.`；`npx vitest run` 334 文件 / 2985 用例全过 |
| 2026-07-30 | **contradiction 此前结构性修不动**（真实 critical：`black-death-and-silk-road` 与 `black-death-europe` 对 1377 拉古萨检疫法各说「来自疫区」与「来自非流行地区」，两次 Fix 都 `skipped`）：写白名单按 `finding.pageSlug` 单页开槽，而对侧页只存在于 `evidence[]` 里，模型判对了错在哪边、连试 `wiki_patch` + `wiki_update` 两次都被 `[PAGE_OUT_OF_SCOPE]` 拒掉，最终零写入 → 判 `skipped` → UI 显示「无需更改」。**与 7-29 orphan 同族：处置的写入落点 ≠ `finding.pageSlug`。** 四处改动：(1) 新增 `fix-deterministic::fixWritableSlugs`，contradiction 并入 `evidence[].pageSlug`（`missing-crossref` 不扩，其落点就是源页；`evidence` 缺失的旧快照回退单页）；(2) `buildPerFindingOutcomes` 的触达判据同口径改为「该 finding 任一可写页被触达」，否则只写对侧页的成功修复仍会被记成 skipped；(3) `scopeFixWrites` 闭包记账被拒 slug 并 emit `fix:scope-blocked`，被拒且零写入的 LLM 类 finding 记 `failed` 而非 `skipped`（`fix:tool` 走 `onStepFinish`，拿不到执行结果，此前失败完全无痕）；(4) `remediation-status::isUntouchedSkip` → `isUnresolvedOutcome`，`failed` 与 `skipped` 一样留在列表可重试 —— 不然这次改动会把「留着但骗人」变成「直接消失」。真实验收：新 discovery 后点该行 Fix → 模型一次 `wiki_patch` 落地 `black-death-europe`（「非流行地区」→「疫区」），零 `fix:scope-blocked`，`perFindingOutcomes=fixed`，`semanticStatus=clean`，vault commit `40c4145`，Health critical 2→1。spec/plan 见 `docs/{specs,plans}/2026-07-30-contradiction-fix-write-scope.md` |
| 2026-07-29 | **Tidy 现在能自主修掉孤页**（此前只做到「说真话」，仍需用户手动补链）：(1) 新增 `lint-deterministic::pageHasInboundLinks`，与 `checkOrphanPages` 共用抽出的 `collectInboundSlugs` 口径；(2) `buildCuratePerFindingOutcomes` 的 fixed 判据从 `touchedSlugs.has(finding.pageSlug)` 改为该判定 —— **孤页的修复天然写在源页上，孤页自己永远不在 touchedSlugs 里，旧判据让补链成功也被记成 skipped**（上一轮遗漏的真实缺陷，叠加 `isUntouchedSkip` 后会让复发闭环换形态继续存在）；(3) 新增 `expandScopeWithOrphanSourceCandidates`，带 worklist 时用 `hybridRankSlugs` 取 `ORPHAN_SOURCE_CANDIDATES=5` 个语义候选源页并入 scope（孤页无入链→一跳图邻居对它无用），emit `curate:orphan-candidates`，无 worklist 零检索；(4) `curate:auto`/`manual` profile 加 `wiki.patch`，`curate-tools::patchPage` 经 allowedSet + update cap + `FIDELITY_PROFILES.fix` 忠实度三重护栏（fix/query 的 patch 均无忠实度护栏，Curate 无人复核才补）。真实验收：`mongol-empire` 跑一次 Tidy → 模型在 `global-history-from-states-to-modernity` 的既有平行列表里加一个 bullet 带 `[[mongol-empire]]`，`perFindingOutcomes=fixed`，确定性重扫零 orphan。spec/plan 见 `docs/{specs,plans}/2026-07-29-curate-orphan-autofix.md` |
| 2026-07-28 | 修复 Tidy 修 orphan 的两处断层：(1) `curate-service` 解析出的 orphan worklist 此前只用于结果归因、从不进 prompt，模型在信息真空里按 `When in doubt, do NOTHING` 稳定零写入 —— 现经 `buildCurateAgenticUserPrompt` 的新 `orphans` 参注入 user prompt（页面/description/suggestedFix，**不含 finding ID**），并明确「邻域无自然锚点就不要写，只报告原因」，无 worklist 时 prompt 逐字节不变；(2) `remediation-status.ts` 新增 `isUntouchedSkip`，Fix/Curate 的 `skipped`（未触达）不再随 fixed/failed 一起从快照移除，改为留在列表并标注 `status:'skipped'` 且不进 `recentOutcomes` —— 此前「点 Tidy 消失 → 重新 lint 原样出现」的翻转闭环由此消除（Research 的 dismissed/empty skipped 属已处理，仍隐藏）。真实 `mongol-empire` 载荷已固化为回归用例。spec/plan 见 `docs/{specs,plans}/2026-07-28-curate-orphan-loop.md` |
...
| 2026-06-27 | Cognitive Lens：新增 `reshape-service.ts`（整页重塑）+ `apply-signal.ts`；均为读侧，不写 vault/不经 Saga |
