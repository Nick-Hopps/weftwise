...
| `src/server/agents/` | Ingest multi-agent runtime + 全 runner 共用的 30 个 builtin tool、8 个 ToolProfile 与执行策略 | [查看](./src/server/agents/CLAUDE.md) |
...
> 已覆盖：既有 Wiki/Saga/agents/ingest/DB/search/lint/Health 全链路，以及 30 个 builtin 的 registry/Profile/compile policy、跨 Subject 只读/复合引用、History list/diff/revert 审批、workflow status/start/cancel 审批、Ask AI canonical 完整块选区配图审批/原子 job/页面+资产 Saga、`wiki.move` 页面身份迁移（alias/旧 URL/跨 Subject 链接/source sidecar/派生缓存/回滚）、frontmatter emoji/fenced code/CRLF 往返与跨 Subject 同名 title resolver、metadata/link 窄写纯函数与 plan/apply、Fix/Curate 串行 Guard、Query PendingAction preview/approve/stale、Research run 原子批准/候选租约/Ingest lineage/验证对账/Health 恢复、逐跳 SSRF 防护、旧 CHECK 原子迁移和 job/embedding + applied 原子最终化；worker 心跳生命周期、双进程 job claim、到期租约、Saga 失败终态/事件顺序、job_events 同毫秒续播、pages 复合 PK/path unique 与手动 FTS update/delete 一致性也已由真实 SQLite/WAL 测试锁定。
...
- 小型维护变更可直接在当前分支完成；新功能基于当前分支创建 `feat/<需求描述>` worktree。
- commit message 使用英文 Conventional Commit 前缀 + 中文一句话摘要，例如 `feat: 新增页面证据工具`、`fix: 修复任务终态竞态`、`docs: 同步架构文档`。
- 特性分支使用 `--no-ff` 合回基线；merge message 使用 `merge: 合并 feat/<需求描述>：<中文摘要>`，合并后删除 worktree 与分支。
- 禁止使用 `功能：`、`修复：`、`文档：`、`重构：`、`测试：`、`合并 feat/...` 等中文类别前缀。

### 版本号自动化（SemVer，beta 阶段）

- `post-commit` hook（`.githooks/post-commit` → `scripts/version/post-commit-bump.ts`）在提交后按类型自增 `package.json` 版本并 amend 回同一提交（SHA 会重写一次）：beta 阶段（major = 0）`feat:` → minor +1、`fix:` → patch +1，其余类型不变；升 1.0.0 是人工决策（此后 `feat!:`/BREAKING CHANGE 自动升 major）。
- 自增规则唯一真实源为 `scripts/version/compute-next-version.ts`（纯函数，有单测）；hook 经 `npm install` 的 `prepare` script 自动接线（`core.hooksPath .githooks`）。
- merge / rebase / cherry-pick、`reset --soft` 压缩重建、amend 均不会重复自增；暂存区不干净时安全跳过；特殊场景用 `SKIP_VERSION_BUMP=1 git commit ...` 显式跳过。设计稿见 `docs/specs/2026-07-20-semver-version-automation.md`。
