...
| `tool-context.ts` | `ToolContext` 接口定义（当前 Subject 证据 + Query 专用跨 Subject/History/workflow 状态只读 + 可选页面/move/History/workflow preview 与 `onPendingAction` + metadata/link 窄写等 worker 能力）；不再暴露原始 `AgentContext` 逃生舱。Query propose 只注入预览服务，Fix/Curate 实际写能力仍按 profile 与 Guard 限制 |
...
| `builtin/subject-list.ts` | `subject.list` — Query 专用，列出 Subject 稳定 slug 与非 meta pageCount，不切换 active Subject |
| `builtin/wiki-search-cross-subject.ts` | `wiki.search_cross_subject` — Query 显式指定 1–5 个其他 Subject 的只读混合检索，结果总带 subjectSlug |
| `builtin/wiki-read-cross-subject.ts` | `wiki.read_cross_subject` — Query 按 subjectSlug + slug 读取其他 Subject 的已提交非 meta 正文，供跨主题引用核查 |
...
└── tools/
    ...
    └── builtin/
        ...
        ├── subject-list.ts         # subject.list（Query-only）
        ├── wiki-search-cross-subject.ts # 跨 Subject 显式只读检索
        ├── wiki-read-cross-subject.ts   # 跨 Subject 正文读取
        ...
        ├── wiki-move.ts            # wiki.move（页面身份迁移 PendingAction）
