...
- 工作目录为 worktree `../weftwise-feat-health-fix-global-context`（分支 `feat/health-fix-global-context`）；commit message 用中文。
...
Run:
```bash
cd /Users/nickhopps/Documents/playground/weftwise-feat-health-fix-global-context
[ -e node_modules ] || ln -s ../weftwise/node_modules node_modules
```
Expected: 无输出（成功）；`ls -la node_modules` 显示符号链接指向 `../weftwise/node_modules`。

- [ ] **Step 2: 冒烟验证依赖可解析**

Run:
```bash
cd /Users/nickhopps/Documents/playground/weftwise-feat-health-fix-global-context && npx vitest run src/server/services/__tests__/fix-deterministic.test.ts 2>&1 | tail -5
```
...
Run:
```bash
cd /Users/nickhopps/Documents/playground/weftwise-feat-health-fix-global-context && npx vitest run src/server/services/__tests__/fix-deterministic.test.ts 2>&1 | tail -15
```
...
Run:
```bash
cd /Users/nickhopps/Documents/playground/weftwise-feat-health-fix-global-context && npx vitest run src/server/services/__tests__/fix-deterministic.test.ts 2>&1 | tail -10
```
...
Run:
```bash
cd /Users/nickhopps/Documents/playground/weftwise-feat-health-fix-global-context && npx vitest run src/server/llm/prompts/__tests__/fix-prompt.test.ts 2>&1 | tail -15
```
...
Run:
```bash
cd /Users/nickhopps/Documents/playground/weftwise-feat-health-fix-global-context && npx vitest run src/server/llm/prompts/__tests__/fix-prompt.test.ts 2>&1 | tail -10
```
...
Run:
```bash
cd /Users/nickhopps/Documents/playground/weftwise-feat-health-fix-global-context && npx tsc --noEmit 2>&1 | tail -20
```
...
Run:
```bash
cd /Users/nickhopps/Documents/playground/weftwise-feat-health-fix-global-context && npx vitest run 2>&1 | tail -15
```
...
Run:
```bash
git -C /Users/nickhopps/Documents/playground/weftwise status --short
```
Expected: 仅 `?? IDEAS.md`（改动应全在 worktree 内）。若出现本特性相关文件，说明写入泄漏，需 `git -C .../weftwise restore` 复位。

- [ ] **Step 4: 回合主分支并清理 worktree**

```bash
cd /Users/nickhopps/Documents/playground/weftwise
git merge --ff-only feat/health-fix-global-context
# 删除 node_modules 软链后移除 worktree
rm -f /Users/nickhopps/Documents/playground/weftwise-feat-health-fix-global-context/node_modules
git worktree remove /Users/nickhopps/Documents/playground/weftwise-feat-health-fix-global-context
