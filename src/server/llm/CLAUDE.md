...
| `query-prompt.ts` | 回答用户问题 + 引用；`buildQueryUserPrompt` 加可选 history 参注入多轮 transcript。`QueryResponseSchema`。`buildQueryAgenticSystemPrompt({ mode, imageInsertEnabled })` 按真实 ToolSet 生成工具说明，并要求 active Subject 引用使用 `[[slug]]`、显式跨 Subject 引用使用 `[[subject:slug]]`，且只能引用本轮 current/cross read 真正读过的正文；跨 Subject 工具严格只读，preview 始终写 active Subject。另含选区意图 `SelectionIntentSchema/SELECTION_INTENT_SYSTEM_PROMPT`、`buildAgenticUserContent` 与独立 coverage 判定 `CoverageSchema/COVERAGE_SYSTEM_PROMPT/buildCoverageUserPrompt`。 |
...
| 2026-07-17 | Query 新增选区意图结构化分类 schema/prompt，复用 `query` task 输出 `image-insert/other`；Agentic system prompt 改为按 read/propose/配图能力动态构造，模型不再看到未实际下发的 mutation 工具 |
