...
手动 Health check 使用 `discovery`，执行确定性扫描与开放式语义发现。Fix/Curate 完成后不再创建 lint job：任务内 `verifyJobPostconditions` 已产出 `perFindingOutcomes`，`remediation-status.ts` 在读取原 lint 快照时把 baseline 之后已完成验证且**确实触达过页面**（fixed/failed）的关联 finding 移除并重算 severity，真实结果保留在近期摘要；**未触达的 skipped 留在列表并标注**（见下方状态恢复段）。Research 等 provenance run 到达验证终态后执行同样投影。若用户之后手动 discovery 又发现同一 finding，较新的 lint 结果优先并重新展示。`/api/lint` 的显式 `verification` 模式与 `lint-verification.ts` 暂留作旧客户端兼容，不再由 Health UI 调用。
