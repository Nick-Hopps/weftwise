...
- `health-view.tsx` —— 🆕 知识库体检工作台；复用 `workspace-page` 的统一宽度、页头、五项摘要、sticky 工具栏和状态区；消费 `HealthSnapshot.remediations` 驱动逐条/批量动作；**门控粒度是「action + target」而非 action 类型**（target = finding ID 或 `BATCH_TARGET`）：工具栏三个批量按钮互不阻塞、lint 运行中也不禁用（`remediationButtonDisabled` + 只含批量哨兵的 `batchBusyActions`），且**不会被逐条行的点击禁用**；逐条行各自门控，禁用只看「本行是否被在途 job 覆盖」（`coveredFindingIds`，服务端 active jobs 派生）。`submittingTargets` 只覆盖「POST 已发出、active-jobs 未回」这一窗口，`runRemediation` 等两次 invalidate 落地后才 `releaseAction`，避免「锁已放、事实未到」的空窗；批量研究按 `MAX_RESEARCH_BATCH_JOBS=10` 取快照前 N 条并显示 `研究 (10 / 23)`，同一 action 的多个在途 job 由 `selectRecoverableHealthJobs`（**全部保留、createdAt 升序**，队首即 worker 正在跑的那个）驱动单条 SSE 逐个观察（**切换观察头前必须 `reset()` 流**——`useJobStream` 在 jobId 变化时不重置 status，残留终态会让下一个 job 瞬间误判跑完；curate/fix/re-ingest/research 四条流都遵守）；「整理/修复/研究」在自己那一批 pending/running 时原位切为 Stop（research 批量的 Stop 取消整批），**逐条行也有自己的 Stop**（`findFindingJob` 取 jobId，pending 也能取消，只影响自己那一个），刷新恢复出的 active job 同样可取消，失败保留可重试入口；Fix/Curate 完成后仅刷新已物化 postcondition 的 Health 快照投影，不再自动入队 lint；Research job 完成后由 `result.runId` GET 持久化 run，按 candidate ID 批准/忽略并轮询 importing/legacy verifying，终态刷新 pages/lint-latest/active jobs
- `finding-row.tsx` —— 单条 finding 采用稳定的摘要/状态/动作三段布局；`disabledActions` 是**父级按本行算好的**不可点集合（不是「哪些 action 类型在忙」），`runningAction` 命中本行时该按钮原位切为 Square + Stop 并可显示 cancelling；`awaiting-approval` 的 research plan 带 `runId` 且挂了 `onReviewRun` 时渲染 `Review candidates` 并**取代**重复的 `Research topic`（缺 `runId` 不猜测 run 身份，只读视图不渲染入口），长描述限制两行，建议与 plan reason 按需展开；内部 status 映射为用户态文案（如 `awaiting-approval` → `Needs action`）；无 plan 时显示 `Plan unavailable`，不按 finding type 猜测动作；`review-source` 只导航
- `remediation-ui.ts` —— 客户端纯 helper：从服务端 actions 收集 finding IDs、**按 `action + target` 开槽的 action gate**、subject/generation origin 隔离、active job 严格解析与 hydration busy；`readStrictRemediationContext` 返回已校验的 `findingIds`（原先校验完就丢，逐条门控因此只能退化为整类禁用），`coveredFindingIds` / `findFindingJob` 据此从服务端在途 job 派生「哪条 finding 正被处理、它的 jobId 是什么」，`rowActionDisabled` 收敛单行判定（`hydrationBusy` 仍整类禁用——首次 hydrate 前防的是刷新后重复提交）；恢复 remediation job 只恢复 workflow，不排队修后 lint；统一派生处置按钮的 idle/starting/running/cancelling 四态并解析通用 job cancel 响应（409 按已终态幂等收敛）；Fix 完成摘要只统计 `perFindingOutcomes`，不把 `writes` 解释为 finding 或页面数；同时严格解析 Research job locator/run view，并构造不含 URL 的批准 body
...
**逐条互不阻塞**：服务端按 `remediationContext`（sorted findingIds）算幂等键，不同 finding 建**独立 job**；worker 对非 ingest job 串行独占执行（`decideClaim`），vault 写入另有 vault-mutex。所以并发点击本就安全，客户端不得按 action 类型整类禁用。`persistedBusyActions` / `blockingRecoverableActions` 退为**只供状态文案与运行态呈现**（`workflowBusyActions`），不再参与任何禁用判定。刷新后客户端不知道在途 job 原本是批量还是逐条，`RecoverableHealthJob.target` 按「只覆盖一条 finding 就归给那一条」推断——只影响工具栏是否显示 Stop，行内 Stop 走服务端事实不依赖它；恰好只选一条的批量被推断成逐条是无害的。
