...
| `src/server/sources/` | 上传文件解析与持久化 + URL 引用实体 + worker 侧 SSRF-safe 临时抓取（URL 不落 raw HTML） | [查看](./src/server/sources/CLAUDE.md) |
...
> vitest 已配置（`vitest.config.ts`），测试文件分布在各模块 `__tests__/` 目录（334 文件 / 2985 用例，2026-07-31）。
