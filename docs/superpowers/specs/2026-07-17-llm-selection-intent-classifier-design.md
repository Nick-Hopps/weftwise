...
3. canonical 选区的配图执行意图进入独立 `image-insert` mode；Reshape 选区的相同意图继续确定性拒绝。
...
解析请求
  ...
  -> image-insert + canonical：image-insert mode
...
统一结构化意图分类上线后，canonical 选区配图曾被映射到通用 `query:propose`。这会让主 Query 同时向 provider 注册 `wiki.image.insert`、`wiki.preview_change` 等全部审批工具。`wiki.preview_change` 的输入是顶层 `z.discriminatedUnion`；部分 OpenAI-compatible provider 将其转换成根节点 `type: null`，在模型开始生成前即拒绝整批工具，报错为 `Invalid schema for function 'wiki_preview_change'`。

最终实现把 Query mode 收口为单一真实源：

- `read`：只读工具，使用 `query:read` policy；
- `propose`：只读工具加通用 PendingAction 提案工具，使用 `query:propose` policy；
- `image-insert`：只读工具加 `wiki.image.insert`，复用 `query:propose` policy，但不携带 `wiki.preview_change` 或其他通用提案工具。

Route 不再维护 `mode + imageInsertEnabled` 两套能力状态。结构化意图为 `image-insert` 且选区通过 canonical 校验时，直接选择 `image-insert` mode；Reshape 仍在进入主 Query 前确定性拒绝。Prompt、ToolDef 集合与 policy 均由同一个 mode 派生。

配图专用 ToolSet 增加 provider schema 回归：每个工具经 AI SDK 的 `zodSchema(...).jsonSchema` 转换后，根节点必须为 JSON Schema `type: "object"`。该约束确保配图请求不会被无关的通用提案 schema 阻断。
