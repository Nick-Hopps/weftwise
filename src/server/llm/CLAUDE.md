...
| `query-prompt.ts` | 回答用户问题 + 引用；`buildQueryUserPrompt` 加可选 history 参注入多轮 transcript。`QueryResponseSchema`。**agentic 工具循环新增**：`QUERY_AGENTIC_SYSTEM_PROMPT`（指示模型使用工具自驱检索，不预载上下文）+ `buildAgenticUserContent(question, ctx, opts?)` （构造工具循环用的用户消息，可选 `currentPageSlug` 提示当前页）。 |
