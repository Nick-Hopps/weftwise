...
| `agents/runtime/agent-loop.ts::runAgent` | 既有 `generation.inputTokens/outputTokens`（ingest 各阶段绕过 provider-registry 直调 AI SDK，是最大头消耗，必须单独埋点；task 取 `route.task`、model 取 `route.model`） | 生成成功后、`chargeTokens` 旁同步记 |
