...
- `health-view.tsx` —— 🆕 知识库体检工作台；复用 `workspace-page` 的统一宽度、页头、五项摘要、sticky 工具栏和状态区；消费 `HealthSnapshot.remediations` 驱动逐条/批量动作；三个处置按钮**互不阻塞**（只受自身 in-flight 门控，lint 运行中也不禁用，判定收敛在纯函数 `remediationButtonDisabled`）；批量研究按 `MAX_RESEARCH_BATCH_JOBS=10` 取快照前 N 条并显示 `研究 (10 / 23)`，拆出的多个 job 由 `researchQueueRef` 逐个用 SSE 观察（worker 本就串行独占，队列排空前不释放 research 锁；**切换 job 前必须 `reset()` 流**——`useJobStream` 在 jobId 变化时不重置 status，残留终态会让整批瞬间误判跑完）；「整理/修复/研究」在 pending/running 时原位切为 Stop（research 的 Stop 取消整批），刷新恢复出的 active job 同样可取消，成功后等待权威 SSE 终态释放 action gate，失败保留可重试入口；Fix/Curate 完成后仅刷新已物化 postcondition 的 Health 快照投影，不再自动入队 lint；Research job 完成后由 `result.runId` GET 持久化 run，按 candidate ID 批准/忽略并轮询 importing/legacy verifying，终态刷新 pages/lint-latest/active jobs
- `finding-row.tsx` —— 单条 finding 采用稳定的摘要/状态/动作三段布局；`awaiting-approval` 的 research plan 带 `runId` 且挂了 `onReviewRun` 时渲染 `Review candidates` 并**取代**重复的 `Research topic`（缺 `runId` 不猜测 run 身份，只读视图不渲染入口），长描述限制两行，建议与 plan reason 按需展开；内部 status 映射为用户态文案（如 `awaiting-approval` → `Needs action`）；无 plan 时显示 `Plan unavailable`，不按 finding type 猜测动作；`review-source` 只导航
