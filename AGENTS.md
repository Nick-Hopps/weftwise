...
> vitest 已配置（`vitest.config.ts`），测试文件分布在各模块 `__tests__/` 目录（239 文件 / 2103 用例，2026-07-14）。
> 已覆盖：既有 Wiki/Saga/agents/ingest/DB/search/lint/Health 全链路，以及 28 个 builtin 的 registry/Profile/compile policy、跨 Subject 只读/复合引用、History list/diff/revert 审批、workflow status/start/cancel 审批、`wiki.move` 页面身份迁移（alias/旧 URL/跨 Subject 链接/source sidecar/派生缓存/回滚）、metadata/link 窄写纯函数与 plan/apply、Fix/Curate 串行 Guard、Query PendingAction preview/approve/stale、Research run 原子批准/候选租约/Ingest lineage/验证对账/Health 恢复、逐跳 SSRF 防护、旧 CHECK 原子迁移和 job/embedding + applied 原子最终化；worker 心跳生命周期、双进程 job claim、到期租约、pages 复合 PK/path unique 与手动 FTS update/delete 一致性也已由真实 SQLite/WAL 测试锁定。
