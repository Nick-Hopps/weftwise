...
| 2026-07-09 | Ask AI 表格渲染 + 引用列表折叠 | `renderMarkdown()`（`src/lib/markdown-client.ts`）接入完整 `remark-gfm`（表格/删除线/任务列表/自动链接），插入在 `remarkParse`/`remarkFrontmatter` 之后、`createRemarkCallouts`/`createRemarkWikiLinks`/`createRemarkMermaid` 等自定义插件之前；chat（`message-list.tsx`）与 Wiki 阅读页正文等所有共用该渲染函数的消费方一并获得表格渲染能力。`message-list.tsx` 新增 `MessageCitations` 组件，引用列表支持展开/折叠（仿 `layout/sidebar.tsx` 现有 "Sources" 分组折叠模式）：超过 3 条默认折叠、≤3 条默认展开，每条消息独立维护本地状态。spec/plan 见 `docs/superpowers/{specs,plans}/2026-07-09-ask-ai-table-citations-collapse*`。|
