...
import { retryResearchRunImport } from '@/server/services/research-approval-service';
import { researchRunErrorResponse } from '../../error-response';

export const runtime = 'nodejs';

const RetryResearchRunBodySchema = z.object({
...
export async function POST(
  ...
  const parsed = RetryResearchRunBodySchema.safeParse(rawBody);
  if (!parsed.success) {
    return NextResponse.json({ error: 'Invalid retry request.' }, { status: 400 });
  }
  ...
  try {
    const result = retryResearchRunImport({
      runId: id,
