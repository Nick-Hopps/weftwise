...
- 删除页面时必须同步删除该页的 `page_renditions` 与 `page_rendition_assets`，避免同 slug 重建后复活旧阅读版本。
- 页面移动时必须同时迁移 rendition 与资产的 slug 归属；Refresh 只能留下新版本实际引用的资产。
- Subject reset 与全局 reset 必须在同一清理事务中删除 rendition 正文和图片。

### 成功产物判定

- 整页 Reshape 只有在最终 Markdown 非空时才算成功；工具轮次耗尽但未产出正文视为生成失败，不得覆盖旧版本。
- 只持久化最终 Markdown 实际引用的 `/api/rendition-assets/<id>`；已生成但未引用的图片留在请求内存中并随请求结束释放。
- Markdown 不得引用本次请求未生成的 rendition asset ID，避免保存悬空或越权拼接的图片 URL。

### stale 版本体验

- 保存版的 canonical hash 或画像版本不匹配时仍允许查看，但状态行必须明确显示该版本基于旧内容或旧画像。
- stale 提示沿用现有细状态行：使用克制的警示图标与短文案，不新增卡片、横幅或弹窗；Refresh 与 Show original/reshaped 保持原位置。
