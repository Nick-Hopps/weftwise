...
CREATE TABLE `__new_pending_actions` (
	...
	CONSTRAINT "pending_actions_operation_check" CHECK("__new_pending_actions"."operation" IN ('create','update','patch','delete','reenrich','metadata-patch','link-ensure','history-revert','workflow-reenrich-start','workflow-research-start','workflow-cancel','move')),
...
CREATE INDEX `pending_actions_status_expiry_idx` ON `pending_actions` (`status`,`expires_at`);--> statement-breakpoint
DELETE FROM `page_aliases`
WHERE rowid NOT IN (
	SELECT MAX(rowid) FROM `page_aliases` GROUP BY `subject_id`, `old_slug`
);--> statement-breakpoint
CREATE UNIQUE INDEX `page_aliases_subject_old_unique` ON `page_aliases` (`subject_id`,`old_slug`);
