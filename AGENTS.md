...
| `src/server/services/` | 任务/工作流编排：ingest/query/lint/fix/curate/re-enrich/embedding + PendingAction 审批（强制 subjectId） | [查看](./src/server/services/CLAUDE.md) |
...
> vitest 已配置（`vitest.config.ts`），测试文件分布在各模块 `__tests__/` 目录（209 文件 / 1753 用例，2026-07-13）。
> 已覆盖：既有 Wiki/Saga/agents/ingest/DB/search/lint/Health 全链路，以及 17 个 builtin 的 registry/Profile/compile policy、metadata/link 窄写纯函数与 plan/apply、Fix/Curate 串行 Guard、Query PendingAction preview/approve/stale、旧 CHECK 原子迁移和 embedding job + applied 原子最终化。
