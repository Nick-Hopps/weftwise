...
- [x] **Step 1: 写 canonicalization 与类型边界的失败测试**
...
- [x] **Step 3: 在 contracts 中定义唯一共享类型**
...
- [x] **Step 4: 实现稳定 canonical JSON、hash 与 Zod 规范化**
...
- [x] **Step 3: 添加 Drizzle schema 与幂等运行时建表**
...
- [x] **Step 3: 在获取 mutex 后、创建 operation 前校验 HEAD**
...
- [x] **Step 1: 写无外部依赖的 unified diff 失败测试**
...
- [x] **Step 2: 写 planner 无副作用与稳定输出的失败测试**
...
- [x] **Step 4: 抽出 reenrich 的纯工作流 planner**
...
- [x] **Step 5: 运行 approval 与 operation 回归**
...
- [x] **Step 1: 写工具注册、Profile 隔离和意图分类失败测试**
...
- [x] **Step 2: 写 builtin handler 与 Query context 回调失败测试**
...
- [x] **Step 4: 注册 query-only preview 工具并扩展 ToolContext**
...
- [x] **Step 6: 更新 Query prompt 的提案语义并运行测试**
...
- [x] **Step 3: 在 stream start 内接线 mode 与 pending-action**
...
- [x] **Step 4: 运行 route 与 Query service 测试**
...
- [x] **Step 1: 写 actionId upsert 与会话替换失败测试**
...
- [x] **Step 4: 重命名 reset 状态并接入 SSE upsert**
...
- [x] **Step 6: 修正 useApiFetch 写请求说明并运行验证**
...
- `npm test -- --run`：186 个测试文件、1233 个用例全部通过。
- `npm run lint`：补充仓库缺失的官方 Strict ESLint 配置后退出码 0；0 error，保留 12 条既有 warning 待后续专项清理。
- `npx tsc --noEmit`：通过。
- `npm run build`：Next.js 15.5.14 生产构建成功，新增三个 pending-actions route 均进入构建清单。
- `npx vitest run src/server/llm/__tests__/config-example.test.ts`：3 个用例通过；`llm-config.example.json` 无差异，本阶段复用现有 `query` 路由，无需新增模型任务。
- Query 安全不变量：`query:read`/`query:propose` 均不含 create/update/patch/delete/reenrich 实际写工具；propose 只增加 `wiki.preview_change`。
- 全量验收期间发现并修复 worker 的循环依赖：审批维护拆到轻量 `pending-action-maintenance.ts`，避免 `worker → pending-action-service → embedding-service → worker` 初始化环。
